...
const ProjectDetail = () => {
  ...
  // Determine which tabs to show based on role and assignment
  const isAssignedFreelancer = role === "freelancer" && user && project.assignedFreelancerIds.includes(user.id);
  const showAllTabs = role === "admin" || isAssignedFreelancer;
...
const InventoryTab = ({ project, role, isEditable, isLocked, profiles }: { project: Project; role: string; isEditable: boolean; isLocked: boolean; profiles: Map<string, FreelancerProfile> }) => {
  ...
  const [flowerInventory, setFlowerInventory] = useState<FlowerInventoryRow[]>(project.flowerInventory);
  const [hardGoodInventory, setHardGoodInventory] = useState<HardGoodInventoryRow[]>(project.hardGoodInventory);
  const filterOptions = getFilterOptions(role);

  const handleFlowerCsvUpload = async (rows: string[][]) => {
    try {
      if (!rows || rows.length === 0) {
        toast.error("CSV file is empty or only contains a header row");
        return;
      }

      console.log("Parsing CSV rows:", rows);

      // Parse CSV rows: [flower, color, stems_in_recipe, total_ordered, extras]
      const parsedFlowers = rows.map((row, index) => ({
        project_id: project.id,
        flower: (row[0] || "").trim(),
        color: (row[1] || "").trim(),
        stems_in_recipe: parseInt(row[2], 10) || 0,
        total_ordered: parseInt(row[3], 10) || 0,
        extras: parseInt(row[4], 10) || 0,
        sort_order: index,
      }));

      console.log("Parsed flowers:", parsedFlowers);

      // Delete existing flower inventory for this project
      const { error: deleteError } = await supabase.from("flower_inventory").delete().eq("project_id", project.id);
      if (deleteError) {
      ...
      const newInventory: FlowerInventoryRow[] = (data || []).map((row: any) => ({
        id: row.id,
        flower: row.flower,
        color: row.color,
        stemsInRecipe: row.stems_in_recipe,
      ...
      setFlowerInventory(newInventory);
      toast.success(`Uploaded ${parsedFlowers.length} flower items`);
    } catch (error) {
      console.error("Error uploading flower inventory:", error);
      toast.error("Failed to upload flower inventory");
    }
  };

  const handleHardGoodCsvUpload = async (rows: string[][]) => {
    try {
      if (!rows || rows.length === 0) {
        toast.error("CSV file is empty or only contains a header row");
        return;
      ...
      // Parse CSV rows: [item, quantity]
      const parsedGoods = rows.map((row, index) => ({
        project_id: project.id,
        item: (row[0] || "").trim(),
      ...
      console.log("Parsed hard goods:", parsedGoods);

      // Delete existing hard goods inventory for this project
      const { error: deleteError } = await supabase.from("hard_good_inventory").delete().eq("project_id", project.id);
      if (deleteError) {
        console.error("Delete error:", deleteError);
        throw deleteError;
      }
      ...
      // Update local state with properly typed data
      const newInventory: HardGoodInventoryRow[] = (data || []).map((row: any) => ({
        id: row.id,
        item: row.item,
      ...
      setHardGoodInventory(newInventory);
      toast.success(`Uploaded ${parsedGoods.length} hard good items`);
    } catch (error) {
      console.error("Error uploading hard goods inventory:", error);
      toast.error("Failed to upload hard goods inventory");
    }
  };

  const handleDeleteFlowerInventory = async () => {
    try {
      const { error } = await supabase.from("flower_inventory").delete().eq("project_id", project.id);
      if (error) throw error;
      setFlowerInventory([]);
      setShowDeleteConfirm(null);
      toast.success("Flower inventory deleted");
    } catch (error) {
      console.error("Error deleting flower inventory:", error);
      toast.error("Failed to delete flower inventory");
    }
  };

  const handleDeleteHardGoodInventory = async () => {
    try {
      const { error } = await supabase.from("hard_good_inventory").delete().eq("project_id", project.id);
      if (error) throw error;
      setHardGoodInventory([]);
      setShowDeleteConfirm(null);
      toast.success("Hard goods inventory deleted");
    } catch (error) {
      console.error("Error deleting hard goods inventory:", error);
      toast.error("Failed to delete hard goods inventory");
    }
  };

  const hasFlowers = flowerInventory.length > 0;
  const hasHardGoods = hardGoodInventory.length > 0;
  const flowerApproved = flowerInventory.filter((r) => r.status === "approved").length;
  const flowerFlagged = flowerInventory.filter((r) => r.status === "flagged").length;
  const flowerReviewed = flowerInventory.filter((r) => r.status).length;
  const hardGoodApproved = hardGoodInventory.filter((r) => r.status === "approved").length;
  const hardGoodFlagged = hardGoodInventory.filter((r) => r.status === "flagged").length;
  const hardGoodReviewed = hardGoodInventory.filter((r) => r.status).length;
  const flowerAllApproved = hasFlowers && flowerFlagged === 0 && flowerInventory.every((r) => r.status === "approved");
  const hardGoodAllApproved = hasHardGoods && hardGoodFlagged === 0 && hardGoodInventory.every((r) => r.status === "approved");
    ...
    <div className="space-y-4">
      ...
      <div className="bg-card rounded-lg border border-border overflow-hidden">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {role === "admin" && hasFlowers && (
                <span className="text-[10px] font-medium text-muted-foreground">
                  {flowerReviewed}/{flowerInventory.length} reviewed
                </span>
              )}
              {flowerAllApproved ?
            ...
            {role === "admin" && hasFlowers && !isLocked &&
              <div className="flex items-center gap-1">
                <CsvUpload label="Replace" onParsed={handleFlowerCsvUpload} compact />
                <button
        ...
        <div className="p-3">
          {hasFlowers ?
            <FlowerCardList
              rows={flowerInventory}
              role={role as "admin" | "freelancer"}
              filter={flowerFilter}
            ...
            /> :
            role === "admin" && !isLocked ?
              <CsvUpload label="Flowers" onParsed={handleFlowerCsvUpload} /> :
      ...
      <div className="bg-card rounded-lg border border-border overflow-hidden">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {role === "admin" && hasHardGoods && (
                <span className="text-[10px] font-medium text-muted-foreground">
                  {hardGoodReviewed}/{hardGoodInventory.length} reviewed
                </span>
              )}
              {hardGoodAllApproved ?
                <span className="text-[10px] font-semibold text-success bg-success/10 px-2 py-0.5 rounded-full">All Confirmed</span> :
            ...
            {role === "admin" && hasHardGoods && !isLocked &&
              <div className="flex items-center gap-1">
                <CsvUpload label="Replace" onParsed={handleHardGoodCsvUpload} compact />
                <button
        ...
        <div className="p-3">
          {hasHardGoods ?
            <HardGoodCardList
              rows={hardGoodInventory}
              role={role as "admin" | "freelancer"}
              filter={hardGoodFilter}
              onUpdateRow={(rowId, updates) => {
                setHardGoodInventory((prev) =>
                  prev.map((row) => row.id === rowId ? { ...row, ...updates } : row)
                );
              }}
              projectId={project.id}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-foreground/40 backdrop-blur-sm" onClick={() => setShowDeleteConfirm(null)}>
          <div className="bg-card rounded-xl shadow-elevated w-[85vw] max-w-sm p-5 space-y-4" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex gap-2">
              ...
              <button
                onClick={showDeleteConfirm === "flowers" ? handleDeleteFlowerInventory : handleDeleteHardGoodInventory}
                className="flex-1 py-2.5 rounded-lg bg-destructive text-destructive-foreground text-sm font-medium">
                Delete
              </button>
            </div>
