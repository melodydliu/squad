...
import { Camera, X, Globe, Instagram, Building2 } from "lucide-react";
...
const ProfilePage = () => {
  ...
  useEffect(() => {
    ...
    (supabase as any)
      .from("studio_roster")
      .select("joined_at, studios(id, name, logo_url)")
      .eq("freelancer_id", user.id)
      .then(({ data }: { data: any[] | null }) => {
        setMyStudios(
          (data || []).map((r) => ({
            id: r.studios?.id ?? "",
            name: r.studios?.name ?? "Unknown Studio",
            logoUrl: r.studios?.logo_url ?? null,
  ...
  return (
    <AppLayout title="Profile">
      <div className="space-y-5">
        ...
        {/* My Studios — freelancers only */}
        {role === "freelancer" && myStudios.length > 0 && (
          <div className="bg-card rounded-lg border border-border p-4 space-y-3">
            <h3 className="font-display text-sm font-semibold text-foreground">My Studios</h3>
            <div className="space-y-2">
              {myStudios.map((studio) => (
                <div key={studio.id} className="flex items-center gap-3">
                  {studio.logoUrl ? (
                    <img src={studio.logoUrl} alt={studio.name} className="w-9 h-9 rounded-xl object-cover border border-border shrink-0" />
                  ) : (
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground">
                      Joined {new Date(studio.joinedAt).toLocaleDateString()}
                    </p>
                  </div>
                </div>
              ))}
