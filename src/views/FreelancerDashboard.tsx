...
const FreelancerDashboard = () => {
  ...
  const [studiosMap, setStudiosMap] = useState<Map<string, StudioInfo>>(new Map());

  useEffect(() => {
    if (loading) return;
    const studioIds = [...new Set(projects.map((p) => p.studioId).filter(Boolean) as string[])];
    ...
    (supabase as any)
      ...
      .then(({ data }: { data: any[] | null }) => {
        ...
        (data || []).forEach((s) => map.set(s.id, { id: s.id, name: s.name, logoUrl: s.logo_url ?? null }));
        setStudiosMap(map);
      });
  }, [projects, loading]);
          ...
          <div className="space-y-3">
            ...
            {tab === "open" &&
              displayProjects.map((project, i) => {
                ...
                const studio = project.studioId ? studiosMap.get(project.studioId) : null;
                return (
                  <motion.div
                    key={project.id}
                  ...
                  >
                    {studio && (
                      <div className="flex items-center gap-1.5 mb-1 px-1">
                        {studio.logoUrl ? (
                          <img src={studio.logoUrl} alt={studio.name} className="w-4 h-4 rounded object-cover" />
                        ) : (
                          <Building2 className="w-3.5 h-3.5 text-muted-foreground" />
                        )}
                        <span className="text-xs text-muted-foreground font-medium">{studio.name}</span>
                      </div>
                    )}
