...
const AdminDashboard = () => {
  ...
  const { studio, loading: studioLoading, refetch: refetchStudio } = useStudio();
  ...
  return (
    <AppLayout
      ...
      title="Dashboard"
      headerAction={
        <Button onClick={() => router.push("/admin/create")} size="sm" className="rounded-lg gap-1.5 h-8 px-3">
          <Plus className="w-3.5 h-3.5" />
          New
        </Button>
      }
    >
      {!studioLoading && studio && !studio.onboardingCompleted && (
        <StudioOnboarding studio={studio} onComplete={refetchStudio} />
      )}
